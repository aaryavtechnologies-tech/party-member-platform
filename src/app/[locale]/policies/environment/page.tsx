...
export default function EnvironmentPolicyPage() {
  ...
  const t = useTranslations("environmentPolicyPageContent");
    ...
    <main>
      <InnerPageHeader 
        ...
        breadcrumbs={[
          ...
          { label: t("title"), href: "/policies/environment" }
        ]}
      />
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {t.raw("visions").map((vision: string, idx: number) => (
                  <div key={idx} className="flex items-center space-x-3 text-lg font-medium p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-800">
                    <Trees className="w-5 h-5 text-accent flex-shrink-0" />
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-24">
                {t.raw("points").map((point: { title: string; desc: string }, idx: number) => (
                  <div 
                    key={idx}
                    className="bg-white dark:bg-slate-950 p-6 md:p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:border-primary/30 transition-all duration-300 group flex flex-col h-full justify-center"
                  >
