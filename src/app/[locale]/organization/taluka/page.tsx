// src/app/[locale]/organization/taluka/page.tsx
...
export default async function TalukaTeamPage() {
  const admins = await getAdminsByRole(AdminRole.TALUKA_ADMIN);

  const counts = await Promise.all(
    admins.map((admin) =>
      countMembersByLocation({
        state: admin.state ?? undefined,
        district: admin.district ?? undefined,
        taluka: admin.taluka ?? undefined,
      })
    )
  );
  ...
  return (
    <section className="p-6 min-h-screen bg-gray-50 dark:bg-slate-900">
      <h1 className="text-3xl font-bold text-center mb-8 text-primary">Taluka Team</h1>
        ...
        {admins.map((admin, idx) => (
          <div key={admin.id} className="bg-white dark:bg-slate-800 shadow-glass rounded transition hover-lift p-4 flex flex-col items-center">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300 mb-2 text-center">
              {admin.state || "-"} / {admin.district || "-"} / {admin.taluka || "-"}
            </p>
